...
class DetectLines(QtWidgets.QWidget, GuiSaveBase):
    def __init__(self, parent=None):
        ...
        self.rho_control = DoubleSlider(decimals=2, orientation=QtCore.Qt.Horizontal)
        self.rho_control.setSingleStep(0.01)
        self.rho_control.setRange(0.01, 1)
        self.rho_control.setValue(1)
        self.layout.addRow("Rho (Resolution):", self.rho_control)

        self.theta_control = DoubleSlider(decimals=2, orientation=QtCore.Qt.Horizontal)
        self.theta_control.setSingleStep(0.01)
        self.theta_control.setRange(0.01, 1)
        self.theta_control.setValue(1)
        self.layout.addRow("Theta (Resolution):", self.theta_control)

        self.threshold_control = DoubleSlider(
            decimals=0, orientation=QtCore.Qt.Horizontal
        )
        self.threshold_control.setSingleStep(1)
        self.threshold_control.setRange(0, 255)
        self.layout.addRow("Threshold:", self.threshold_control)

        self.min_theta_control = DoubleSlider(
            decimals=0, orientation=QtCore.Qt.Horizontal
        )
        self.min_theta_control.setSingleStep(1)
        self.min_theta_control.setRange(0, 180)
        self.min_theta_control.doubleValueChanged.connect(self.keep_range)
        self.layout.addRow("Min Theta:", self.min_theta_control)

        self.max_theta_control = DoubleSlider(
            decimals=0, orientation=QtCore.Qt.Horizontal
        )
        self.max_theta_control.setSingleStep(1)
        self.max_theta_control.setRange(0, 180)
        self.max_theta_control.doubleValueChanged.connect(self.keep_range)
        self.layout.addRow("Max Theta:", self.max_theta_control)

        self.min_length_control = DoubleSlider(
            decimals=0, orientation=QtCore.Qt.Horizontal
        )
        self.min_length_control.setSingleStep(1)
        self.min_length_control.setRange(0, 100)
        self.layout.addRow("Min Length:", self.min_length_control)

        self.max_gap_control = DoubleSlider(
            decimals=0, orientation=QtCore.Qt.Horizontal
        )
        self.max_gap_control.setSingleStep(1)
        self.max_gap_control.setRange(0, 255)
        self.layout.addRow("Max Gap:", self.max_gap_control)

        self.operations_changed(self.operation.currentText())

    def connect_ui(self, update_func):
        self.rho_control.doubleValueChanged.connect(update_func)
        self.theta_control.doubleValueChanged.connect(update_func)
        self.threshold_control.doubleValueChanged.connect(update_func)
        self.min_theta_control.doubleValueChanged.connect(update_func)
        self.max_theta_control.doubleValueChanged.connect(update_func)
        self.min_length_control.doubleValueChanged.connect(update_func)
        self.max_gap_control.doubleValueChanged.connect(update_func)
    ...
    def process_img(self, img):
        ...
        if operation == "Standard Hough Lines":
            ...
            if linesP is not None:
                for i in range(0, min([len(linesP), 50])):
                    l = linesP[i][0]
                    cv.line(
